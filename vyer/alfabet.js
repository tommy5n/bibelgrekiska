...
export function teardown() {
  if (__kh) {
    document.removeEventListener("keydown", __kh);
    __kh = null;
  }
}
...
export function render(root) {
  ...
  function buildOptions(correct) {
    ...
    if (distractors.length < 3) {
      ...
      distractors = distractors.concat(rest.slice(0, 3 - distractors.length));
    }
  ...
  function renderQuiz() {
    ...
    if (!state.answered) {
      ...
      box.querySelectorAll(".choice").forEach((b) => {
        ...
        else if (b.dataset.name === state.chosen.name) b.classList.add("wrong");
      });
  ...
  function renderPicker() {
    ...
    letters.forEach((l, i) => {
      ...
      cells[i].textContent = state.caseMode === "upper" ? l.upper : l.lower;
    });
  ...
  __kh = (e) => {
    ...
    if (
      ...
      const btn = el("choices").querySelectorAll(".choice")[Number(e.key) - 1];
      if (btn) btn.click();
      return;
    }
